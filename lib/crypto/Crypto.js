...
module.exports = class Crypto {
	...
	/**
	 * EBICS 3.0 (H005) bank public key digest: unlike H004's digestPublicKey()
	 * (a hash of the ASCII hex modulus/exponent), H005's <BankPubKeyDigests>
	 * is the base64-encoded SHA-256 hash of the raw DER-encoded X.509
	 * certificate bytes. Verified against the worked example in the EBICS
	 * Common Implementation Guide (based on EBICS 3.0), section 2.4.4: hashing
	 * the DER bytes of the example <ds:X509Certificate> reproduces the
	 * document's own expected digest exactly.
	 * @param {Key} key - a Key with an attached certificate (see Key#hasCertificate)
	 */
	static digestCertificate(key) {
		return crypto.createHash('sha256').update(key.certificateDer()).digest('base64').trim();
	}
