...
const stringifyKeys = (keys) => {
	Object.keys(keys).map((key) => {
		keys[key] = keys[key] === null ? null : keys[key].toPersistable();
		return key;
	});
...
module.exports = class Client {
	...
	async initialization(order) {
		const keys = await this.keys();
		if (keys === null) await this._generateKeys(order.version);
	...
	ebicsRequest(order) {
		return new Promise(async (resolve, reject) => {
			...
			// H005's OrderDetails carries the order type as 'AdminOrderType'
			// rather than H004's 'OrderType' (see
			// lib/predefinedOrders/h005/*.js) - fall back across both so
			// trace labels work for either version.
			const orderTypeLabel = order.orderDetails.AdminOrderType || order.orderDetails.OrderType;

			if (this.tracesStorage)
				this.tracesStorage
					.label(`REQUEST.${orderTypeLabel}`)
					.data(r)
					.persist();
			...
			(err, res, data) => {
				// Without this `return`, a request-level failure (DNS,
				// TLS, connection refused/blocked, timeout - `data` is
				// undefined in all of these) fell through into
				// `data.toString()` below, throwing an unrelated
				// "Cannot read properties of undefined" TypeError
				// instead of surfacing the actual error via `reject`.
				// Found while live-testing BTD against PostFinance's
				// ISO environment from behind an egress proxy that
				// blocks the destination - the real "proxy denied this
				// connection" error was being masked by this bug.
	...
	/**
	 * @param {string} [version='h004'] - the order's EBICS version
	 *   ('h004'/'h005'), which selects the key material shape: h005 keys
	 *   are additionally wrapped in a self-signed X.509 certificate, as
	 *   required for EBICS 3.0 key initialisation. See Keys.generate().
	 */
	async _generateKeys(version = 'h004') {
		const keysObject = Keys.generate(version || 'h004', { commonName: this.userId });
