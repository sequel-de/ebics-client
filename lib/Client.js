...
module.exports = class Client {
	...
	ebicsRequest(order) {
		return new Promise(async (resolve, reject) => {
			...
			// H005 uses AdminOrderType instead of H004's OrderType.
			...
			(err, res, data) => {
				// `return` is required here: without it a request-level
				// failure (no `data`) fell through into `data.toString()`
				// below, masking the real error with a TypeError.
