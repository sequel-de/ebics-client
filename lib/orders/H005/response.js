...
module.exports = (xml, keys) => ({
	 ...
	 *
	 * Unlike H004, there is no 'PubKeyValue' wrapper element here: per the
	 * real ebics_types_H005.xsd / ebics_signature_S002.xsd, PubKeyInfoType
	 * (and its Authentication/EncryptionPubKeyInfoType extensions) place
	 * 'ds:X509Data' as a DIRECT child of AuthenticationPubKeyInfo /
	 * EncryptionPubKeyInfo, followed by the *Version element - so we select
	 * on 'ds:X509Data' itself rather than a wrapping 'PubKeyValue'.
	 */
	bankKeys() {
		...
		const keyNodes = xpath.select("//*[local-name(.)='X509Data']", doc);
		const bankKeys = {};
